...
export default function edit({ attributes, setAttributes }) {
  const { listTaxonomy, partnerListing, anchor } = attributes;

  // useSelect to retrieve all post types
  const taxonomies = useSelect(
    (select) =>
      select(coreStore).getEntityRecords("taxonomy", "partner-category", {
        per_page: -1,
      }),
    []
  );
  ...
  function onTaxCheckChange(newValue) {
    ...
    data.sort();
    setAttributes({ listTaxonomy: [...data], partnerListing: !partnerListing });
  }
          ...
          <PanelBody title={__("Settings", "smt-theme")}>
            <label className="components-base-control__label custom--label">
              ...
              taxonomyOptions.length > 0
                ...
                : __("Add a Partner Category", "smt-theme")}
            </label>
